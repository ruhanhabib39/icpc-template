...
public:
   HLD(int n_, const vector<vector<int>>& graph) : n(n_), par(n), heavy(n, -1), level(n), root(n), tree_pos(n), tree(n) {
      par[0] = -1;
      level[0] = 0;
      dfs(graph, 0);
      int ii = 0;
...
template<class T, class HLDSegTree>
int HLD<T,HLDSegTree>::dfs(const vector<vector<int>>& graph, int u) {
   int cc = 1, max_sub = 0;
   for(int v : graph[u]) {
      if(v == par[u]) continue;
      par[v] = u;
